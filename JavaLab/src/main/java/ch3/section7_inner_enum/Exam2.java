...
enum Direction2{
    // enum 상수(ex. NORTH)는 enum 클래스의 서브 클래스 객체!!
    NORTH(100), SOUTH(200), EAST(300), WEST(400);

    // 개발자 임의 데이터를 상수에 추가하려면
    // 먼저 enum클래스 생성자 정의하고 매개변수로 데이터 선언
    int price;
...
enum Direction3 {
    NORTH(1){
        ...
        // 개발자가 원한다면 이 상수 클래스만의 함수를 추가 가능
        // 이 함수는 외부에서는 절대 이용이 안됨.. enum 클래스 타입으로 사용되어서!!
        void method(){}
    };
