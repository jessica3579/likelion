...
public class Exam3 extends Super {
    public static void main(String[] args) {
        ...
        // 상속 관계가 없는 다른 패키지의 클래스 이용... ( public 만 이용 가능)
        Two two = new Two();
        two.publicData++;
//        two.protectedData++; // error
//        two.defaultData++; // error
//        two.privateDate++; // error

        Exam3 obj = new Exam3();
        obj.publicSuperData++;
        obj.protectedSuperData++; // 패키지가 다르지만.. 상속관계의 하위라면 이용 가능
...
// 접근제한자는 모든 곳에 모든 제한자가 추가되지는 않음
// 클래스 선언에는 public 과 defaul만 허용됨
class A { } // ok
